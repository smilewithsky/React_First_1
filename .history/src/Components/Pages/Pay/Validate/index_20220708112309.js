...
function Validate({payList , products}){
    ...
    // axios.put(apiname/table/id/giá trị thay đổi)
    const handleSubmit = (e) => {
        const cloneProduct = [...products]
        const clonePayList = [...payList]
        const updateProduct = cloneProduct.map( itemProduct => {
            
            const filterUpdate = clonePayList.filter( payItem => {
                return payItem.idProduct == itemProduct.id
            } )
