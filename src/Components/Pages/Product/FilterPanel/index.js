...
function FilterPanel({setSort , sort , search , setSearch , type , setType , setSale , setRating , rating , sale}) {
        ...
        <div className="filter-group__body">
            {priceProduct.map( (item , index) => (
              <div className='filter-check-box' key={`price-${index}`}>
                <input className='filter-input-check-box' type="checkbox" name="type-product" id={`price-product-${index}`} />
